...
Uses the Fangraphs data export API (JSON) — same endpoint the leaderboard
page calls internally. No HTML parsing required.
...
# stat columns we request — must match Fangraphs column codes exactly.
# Requesting the superset; nulls are acceptable for pitchers with limited data.
PITCHER_STATS = (
...
def _build_record(row: dict, season: int, fetched_at: str) -> dict:
    """Map a raw Fangraphs API row dict to our output schema."""
...
class FangraphsPitchersScraper(BaseScraper):
    """
    Fetches season pitcher leaderboard from Fangraphs JSON API.
    Minimum IP filter (default 10) applied to keep the record set clean.
    Set config['min_ip'] to override.
    """
    ...
    def fetch(self) -> dict:
        ...
        params = {
            ...
            "stats": "pit",
            "lg": "all",
            "qual": str(min_ip),  # Fangraphs qual param accepts integer IP threshold
            "season": season,
            "season1": season,
            "ind": 0,       # combined totals, not split by team
            "team": 0,
            "rost": 0,
            "age": 0,
            "filter": "",
    ...
    def content_key(self, raw: dict) -> str:
        """Hash on pitcher IDs + ERA values — excludes timestamps."""
        rows = raw.get("data", [])
        keys = [
            f"{r.get('playerid')}:{r.get('ERA')}"
            for r in rows
        ]
    ...
    def parse(self, raw: dict) -> list[dict]:
        ...
        # Verify expected columns are present in first row.
        first = rows[0]
