...
STADIUM_COORDS: dict[str, tuple[float, float]] = {
    ...
    "Oakland Athletics": (38.5803, -121.5029),
...
class WeatherScraper(BaseScraper):
    ...
    def fetch(self) -> dict:
        ...
        schedule_params = {
            "sportId": 1, "hydrate": "team,linescore",
        ...
        sched_resp = requests.get(
            ...
            params=schedule_params, timeout=15,
        )
        ...
        for date_entry in schedule.get("dates", []):
            ...
            for game in date_entry.get("games", []):
                ...
                away_name = to_canonical(teams.get("away", {}).get("team", {}).get("name", ""))
                home_name = to_canonical(teams.get("home", {}).get("team", {}).get("name", ""))
                ...
                if is_dome:
                    game_weather.append({
                        "game_id": game_pk, "date": date_str,
                        "away_team": away_name, "home_team": home_name,
                        "stadium": stadium, "city": city, "state": state,
                        "is_dome": True, "is_retractable": False, "weather_raw": None,
                    })
                    continue
                coords = STADIUM_COORDS.get(home_name)
                if not coords:
                    logger.warning("No stadium coords for team '%s' — skipping weather", home_name)
                    game_weather.append({
                        "game_id": game_pk, "date": date_str,
                        "away_team": away_name, "home_team": home_name,
                        "stadium": stadium, "city": city, "state": state,
                        "is_dome": False, "is_retractable": is_retractable, "weather_raw": None,
                    })
                    continue
                try:
                    lat, lon = coords
                    ...
                    game_weather.append({
                        ...
                        "away_team": away_name, "home_team": home_name,
                        "stadium": stadium, "city": city, "state": state,
                        "is_dome": False, "is_retractable": is_retractable, "weather_raw": weather,
                    })
                    logger.debug("Fetched weather for %s @ %s", away_name, home_name)
                except Exception as e:
                    logger.warning("Weather fetch failed for %s @ %s: %s — using nulls", away_name, home_name, e)
                    game_weather.append({
                        "game_id": game_pk, "date": date_str,
                        "away_team": away_name, "home_team": home_name,
                        "stadium": stadium, "city": city, "state": state,
    ...
    def parse(self, raw: dict) -> list[dict]:
        ...
        for g in raw.get("games", []):
            w = g.get("weather_raw") or {}
            wind_dir = _degrees_to_direction(w.get("wind_direction_deg"))
            records.append({
                "game_id": g["game_id"], "date": g["date"],
                "away_team": g["away_team"], "home_team": g["home_team"],
                "stadium": g.get("stadium", ""), "city": g.get("city", ""), "state": g.get("state", ""),
