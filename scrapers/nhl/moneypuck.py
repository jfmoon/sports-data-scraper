...
class NhlGoalieStat(BaseModel):
    ...
    goals_saved_above_average: float | None = None
    xga: float | None = None
...
class MoneypuckScraper(BaseScraper):
    ...
    def _parse_teams(self, csv_text: str, season: str, url: str, fetched_at: str) -> list[dict]:
        if not csv_text:
        ...
        for row in rows:
            ...
            records.append({
                ...
                "source": "moneypuck", "source_url": url, "fetched_at": fetched_at,
            })
        return records

    def _parse_goalies(self, csv_text: str, season: str, url: str, fetched_at: str) -> list[dict]:
        if not csv_text:
        ...
        for row in rows:
            ...
            records.append({
                ...
                "goals_saved_above_expected": _safe_float(row.get("goalsSavedAboveExpected") or row.get("GSAx")),
                "goals_saved_above_average": _safe_float(row.get("goalsSavedAboveAverage") or row.get("GSAA")),
                "xga": _safe_float(row.get("xGoalsAgainst") or row.get("xGA")),
    ...
    def _parse_skaters(self, csv_text: str, season: str, url: str, fetched_at: str) -> list[dict]:
        if not csv_text:
        ...
        for row in rows:
            ...
            records.append({
                ...
                "team": team, "position": row.get("position", ""),
    ...
    def upsert(self, records: list[BaseModel]) -> None:
        ...
        for payload_data, gcs_key, source_name, count in [
            (
                {
                    # Standard envelope — schema_version 1
                    "schema_version": 1,
                    "generated_at": fetched_at,
                    "scraper_key": "moneypuck",
            ...
            logger.info("MoneyPuck: wrote %d records to %s", count, gcs_key)
