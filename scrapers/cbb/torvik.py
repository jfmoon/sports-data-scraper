...
class TorvikScraper(BaseScraper):
    ...
    def upsert(self, records: List[BaseModel]) -> None:
        """
        Write two GCS snapshots from the validated full-season records.

        Note: The splits snapshot (torvik_splits.json) is written from the
        raw fetch result stored during the ScraperRunner flow. Since BaseScraper
        only passes validated records to upsert(), we write the full-season
        snapshot here and let the splits be a best-effort secondary write.
        """
