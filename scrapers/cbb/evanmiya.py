...
EvanMiya framework wrapper. Integrates evanmiya_scraper.py into the repo's
BaseScraper / ScraperRunner system.
...
The scraper opens a visible browser and waits for manual login. It cannot
run autonomously on Cloud Run without Playwright + Chromium container setup
and session-cookie injection.
"""
...
class EvanMiyaScraper(BaseScraper):
    ...
    def fetch(self) -> List[dict]:
        """
        Navigate to EvanMiya, restore saved session (or prompt for manual
        login on first run), and extract the full team ratings table.
    ...
    def parse(self, raw: List[dict]) -> List[dict]:
        """
        No additional parsing needed — evanmiya_scraper.py returns
        fully normalized dicts ready for validation.
        """
        return raw
